//! 数据库迁移 V62：为 blackboards 表新增 enabled 总开关。
//!
//! ## 背景
...
pub(super) struct V62AddBlackboardEnabled;

#[async_trait]
impl Migration for V62AddBlackboardEnabled {
    fn version(&self) -> i64 {
        62
    }

    fn name(&self) -> &'static str {
        "V62AddBlackboardEnabled"
    }

    /// 为 blackboards 表添加 enabled 列，默认 1（启用），保持向后兼容。
    async fn up(&self, db: &Database) -> Result<(), sea_orm::DbErr> {
        ...
        .await?;
        tracing::info!("V62: blackboards.enabled 列已添加");
        Ok(())
    }
}
