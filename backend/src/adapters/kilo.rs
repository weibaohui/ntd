...
mod tests {
    ...
    fn test_extract_session_id_caches_result() {
        let executor = KiloExecutor::new("kilo".to_string());
        let line = r#"{"type":"step-start","sessionID":"kilo_sess_abc"}"#;
        let sid = executor.extract_session_id(line);
        assert_eq!(sid, Some("kilo_sess_abc".to_string()));
        // 再次调用仍能获取缓存值
        assert_eq!(executor.extract_session_id(r#"{"type":"text"}"#), Some("kilo_sess_abc".to_string()));
    }

    #[test]
    fn test_get_session_id_returns_cached() {
        let executor = KiloExecutor::new("kilo".to_string());
        let line = r#"{"type":"step-start","sessionID":"kilo_session_xyz"}"#;
        let _ = executor.parse_output_line(line);
        assert_eq!(executor.get_session_id(), Some("kilo_session_xyz".to_string()));
    }
