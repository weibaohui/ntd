...
2. **数据范围冲突**：后端 `db/dashboard.rs` 的统计 SQL 原本就是全库聚合，没有按 `workspace_id` 过滤；`execution.rs` handler 中也保留过 `TODO(ws-isolation)` 注释，说明 workspace 隔离从未真正落地。

因此 Dashboard 事实上是全局数据，但路由和菜单都伪装成 workspace-scoped，需要归正为真正的全局视图。
...
- CLI `ntd stats` 已移除 `--workspace-id` 参数，现在直接请求全局端点 `/api/v1/stats/dashboard`。
  - 旧命令 `ntd stats --workspace-id <id>` 将因未知参数报错，应改为 `ntd stats`。
  - 若需按 workspace 查看统计，请切换到对应 workspace 后通过「事项 / 看板」等工作区视图查看。
