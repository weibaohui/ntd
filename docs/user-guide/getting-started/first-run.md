...
ntd 本身不执行代码，**必须**先配好至少一个执行器（Claude Code / CodeBuddy / OpenCode / AtomCode / Hermes / Kimi / MobileCoder / Codex / CodeWhale / Pi 等 10 个内置执行器之一）才能跑 Todo。
...
- **建议先开自动备份**：设置 → 备份与恢复 → 数据库 → 自动备份，每天 04:00，保留 30 个文件（默认值）
- **建议先开「执行前确认」**：系统设置 → `execution_timeout_secs` 调成 7200（2 小时），避免执行器卡死
- **工作空间**：在「工作空间」tab 添加你常用的代码目录，Todo 工作目录从白名单选更安全
