...
**Q: 装完后 `ntd` 命令找不到？**
A: PATH 没包含 npm bin 目录。`npm bin -g` 看路径，加到 `~/.zshrc` 或 `~/.bash_profile`。
...
A: 执行器没正常退出。检查 `execution_timeout_secs` 是否太长，或执行器卡死。
...
**Q: 加新的执行器要改代码吗？**
A: 不用。8 个内置执行器开箱即用。新增执行器需要改后端（继承 Executor trait）。
...
**Q: 飞书消息丢失？**
A: ntd 用 message_debounce 5 分钟内同群高频消息去重。看 `feishu_history_messages` 表。
