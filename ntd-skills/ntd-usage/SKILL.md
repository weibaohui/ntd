...
| 想做的事 | 怎么做 |
|----------|--------|
| 注册一个新工作空间 | 在前端「项目目录」面板（侧栏入口 `projectDirectories`）添加目录，拿到自增 ID |
| 查看已有工作空间列表 | 前端「项目目录」面板查看；后端列表接口为 `GET /api/project-directories` |
...
**为什么 CLI 没有注册命令**：工作空间的注册涉及「目录路径录入 + Git Worktree 开关 + 自动清理开关」等多字段配置，CLI 子命令表达不来这套表单，故注册只在前端 UI 做；CLI 仅消费已注册工作空间的 ID。
