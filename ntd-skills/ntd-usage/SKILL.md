...
| "帮我重构 UserService" | 创建 todo → 指定 workspace → 分配执行器 |
...
| "帮我 review 代码变更" | 创建 todo → 指定执行器 → worktree 模式 |
...
2. 收集必要信息：「目标目录是哪里？用哪个 AI 执行器？」
3. 创建 Todo：
   ntd todo create "重构 UserService" --executor claudecode --workspace /path/to/project
4. 执行任务：
...
你的引导流程：
1. 创建 todo 但不立即执行：
   ntd todo create "分析服务日志" --executor claudecode --workspace /var/log
2. 用详细 prompt 执行：
