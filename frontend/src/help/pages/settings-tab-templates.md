...
> **分享到官方仓库**（需求 027）：专家/事项模板/工艺/技能四个子 Tab 的列表行操作区均有「分享」入口——点击后经 ActionButton 唤起 AI 执行器，读取本机 GitCode PAT 把资源提交为 PR 到官方仓库 `weibaohui/ntd-resource`。来源限制：**专家/事项模板/工艺**仅用户自己的资源（非系统同步）可分享；**技能全量可分享**（不区分来源）。PAT 在「设置 → 第三方授权 → GitCode」填写。
