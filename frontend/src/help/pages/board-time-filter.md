...
- **后端入口**：各视图对应 handler 的 `hours` query 参数（`GET /api/v1/workspaces/{ws}/executions` / `/todos/recent-completed` / `/loops/{id}/executions`）
- **注意**：`TIME_RANGE_OPTIONS` 是全站唯一事实源（需求 031），从 `TimeRangeSegmented` 组件导出，本页不再自持一份；`hours = null`（all）表示不按时间过滤，各视图的 cutoff 逻辑用 `if (hours && hours > 0)` 守卫；结论视图把 `hours` 下推到 `getRecentCompletedTodos` 做服务端过滤，运行/环路视图把 `hours` 作为 prop 传入子组件
