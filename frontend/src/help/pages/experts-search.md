...
## 功能位置
专家页 → 搜索栏（`Input` 前缀 `SearchOutlined`，placeholder「搜索专家名称、职业或描述」）
...
## 开发指导
- **前端入口**：`frontend/src/components/settings/ExpertsPanel.tsx` 的 `filteredExperts` useMemo（`useMemo` 依赖 `experts` 与 `searchText`）
