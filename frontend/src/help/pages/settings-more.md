...
更多设置页（SettingsPage）以 `Tabs` 形式承载多个配置面板，Tab 顺序为：系统设置、界面显示、模板管理、备份与恢复、云端同步、关于。执行器管理、会话管理、工作空间、Skills 管理、运行管理已独立为左侧导航菜单项，不再嵌套在设置页的标签页中。Tab 切换通过 `useViewState` 同步 URL，支持 popstate 回退。
