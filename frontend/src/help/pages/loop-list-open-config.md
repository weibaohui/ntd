...
flowchart LR
  ...
  OC --> GD["getWorkspaces()"]
  GD --> API["GET /api/v1/workspaces"]
  API --> H1[workspaces handler]
  H1 --> DB[(workspaces 表)]
...
flowchart TD
  ...
  CFG -->|"getWorkspaces"| DBT["database/todos getWorkspaces"]
  DBT -->|"unwrap"| API["api.get /api/v1/workspaces"]
...
classDiagram
  ...
  UseLoopConfigState --> Workspace : currentWorkspace
  WorkspaceLoopConfigPage --> Workspace : props.workspace
```
...
stateDiagram-v2
  ...
  加载中 --> 列表态: getWorkspaces 失败 message.error
...
- **后端入口**：仅 `getWorkspaces`（`GET /api/v1/workspaces`）取工作空间目录，环路配置页本体是前端 `ReviewTemplatesPanel`（评审模板管理），不直接落环路后端接口。
