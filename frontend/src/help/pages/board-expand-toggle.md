...
看板页 → 结论视图卡片内 `TodoCard` 的点击展开/收起（卡片整体 `onClick` + prompt 区域 `onTogglePrompt`）
...
```mermaid
flowchart TD
  MemorialBoard --> expandedIds["expandedIds: Set<number>"]
  MemorialBoard --> promptExpandedIds["promptExpandedIds: Set<number>"]
...
- **前端入口**：`frontend/src/components/MemorialBoard.tsx` 的 `toggleExpand` 和 `togglePromptExpand` 函数，以及 `expandedIds` / `promptExpandedIds` state
