...
2. 先把 {{expert_dir}} 中的 ~ 展开为当前用户家目录的绝对路径，再遍历该目录，收集全部文件的相对路径与内容；跳过 .downloaded_at、.clawhub、.git 三类同步元数据。
3. 把第 1 步读到的 pat 作为 HTTP 认证令牌，附加到下面每个 GitCode API 请求的认证头里（bearer 认证方式），不要写成占位符：
   ...
   d. 逐文件写入：\`POST https://api.gitcode.com/api/v5/repos/{步骤 a 的 login}/ntd-resource/contents/{path}\`，表单字段 content=<文件字节的 base64>、message="贡献专家 {{expert_name}} v{{version}}"、branch=<步骤 c 的分支名>。
