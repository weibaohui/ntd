...
interface TodoListHeaderProps {
  isMobile: boolean;
  /** 视图模式：card 卡片墙 / list 表格 / running 执行监控（复用 RunningBoard）。 */
...
export function TodoListHeader({
  ...
  const segmented = (
    <Segmented
      size="small"
      value={viewMode}
      onChange={(v) => onViewChange(v as 'card' | 'list' | 'running')}
      options={[
        { value: 'card', icon: <AppstoreOutlined />, title: isMobile ? '卡片' : '卡片视图' },
        { value: 'list', icon: <UnorderedListOutlined />, title: '列表' },
        // 执行监控：复用 RunningBoard（执行记录 6 列 + 实时 WS + 评审流水线），与 card 的 todo 定义维度区分。
