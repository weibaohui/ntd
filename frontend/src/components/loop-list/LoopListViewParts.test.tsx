...
import { buildRowActions, loopProcessName, loopProcessVersion } from './LoopListViewParts';
...
describe('工艺名称/版本列文本', () => {
  ...
  it('工艺安装的环路展示 display_name 与版本快照', () => {
    const loop: LoopListItem = {
      ...makeLoop(),
      process_template_display_name: '标准需求交付',
      process_template_version: '1.2.0',
    };
    expect(loopProcessName(loop)).toBe('标准需求交付');
    expect(loopProcessVersion(loop)).toBe('1.2.0');
  });

  it('display_name 缺失时回退标识名 process_template_name', () => {
    // 040 起 name 不再唯一但仍是稳定标识；display_name 为空时用它兜底展示。
    const loop: LoopListItem = {
      ...makeLoop(),
      process_template_name: '4p12s-delivery',
    };
    expect(loopProcessName(loop)).toBe('4p12s-delivery');
  });

  it('手工环路（无工艺字段）两列均显示 -', () => {
    // 非工艺实例化的环路没有任何来源模板信息，列展示用占位符而非空文本。
    expect(loopProcessName(makeLoop())).toBe('-');
    expect(loopProcessVersion(makeLoop())).toBe('-');
  });
});
