...
> **读取顺序**：`AGENTS.md` → `CLAUDE.md` → 对应技术栈规范（后端/前端）→ 文档编写规范（如需创建文档）。
...
6. **编译+测试** — `cargo clippy -- -D warnings` + `cargo test`
...
> 下节「AI 反思检查清单」是上述原则（尤其第 2、5、6 条）在动手编码前的速查应用形式。

## AI 反思检查清单（代码编写前）

**强制要求：在写任何代码之前，AI 必须按顺序检查下列反思清单（「工程设计原则」的编码前速查）。**
...
- 单个函数体（不含函数签名、空行、注释）原则上不得超过 50 行。
- 50 行是默认推荐值，不是死线。**超过 50 行需要给出明确的豁免理由**，AI 判定是否豁免时应遵循以下原则和清单。

#### 核心原则

允许超 50 行的唯一理由，不是"这段逻辑太复杂"，而是 **"强行拆分将导致数据碎片化或跳转阅读，反而增加理解成本"**。

#### 📋 可豁免的 4 种具体场景（AI 判定清单）

以下 4 类场景允许超出 50 行限制：

1. **纯数据构建（配置/字典/ORM 映射）**：函数主体是连续的对象属性赋值、字典初始化或 Mock 数据填充。**特征**：没有 `if/else` 和循环，全是 `=` 和 `.`。
2. **线性管道（无分支的流水线）**：对输入数据依次执行 A→B→C→D 步，每步结果紧密依赖上一步。**特征**：拆分后每个小函数只被调用一次，且参数传递多达 4-5 个。
3. **密集数学计算/状态机映射**：物理公式、加解密、复杂的坐标变换。**特征**：代码是连续的算术运算，强行拆分会破坏公式的物理完整性。
4. **"卫语句"堆积（线性错误处理）**：如连续 5-8 个 `if err != nil` 后跟简单返回。**特征**：分支没有嵌套，全是平行且提前返回的检查。

#### 🚫 以下情况坚决不予豁免（红线）

无论多少行，只要命中一条就必须重构：

- 嵌套层级 > 3 层（如 `if` 里套 `for` 里套 `if`）。
- 存在多个 `return` 路径且夹杂副作用。
- 同一函数内处理了超过 2 个不同的业务抽象层级（例如既操作 SQL 又拼接 JSON 又发邮件）。

#### 函数拆分补充原则

- 超过 50 行的函数，如不符合上述豁免条件，必须拆分为多个职责单一的子函数，并通过有意义的函数名表达其意图。
